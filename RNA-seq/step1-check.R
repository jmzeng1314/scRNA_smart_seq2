...
group_list=df$g #'$'符，取列，取df矩阵的g列,取出层级聚类信息
...
#对dat矩阵每行求标准差，排序，取最后的1000行，并获得后1000行的行名（探针名）
#sd()求标准差，对dat矩阵每一行的counts求标准差
#sort()函数,排序;
#tail()函数，显示操作对象后面的信息，默认后6行，这里设定取后1000行
#names()函数，获取或设置对象的名称
library(pheatmap)

##画热图,针对top1000的sd的基因集的表达矩阵,没有聚类分组
pheatmap(dat[cg,],show_colnames =F,show_rownames = F,
         filename = 'all_cells_top_1000_sd.png')

##针对top1000的sd的基因集的表达矩阵，归一化，分组画图
if(T){
...
if(T){
  ...
  ##这个聚类分组只是对top1000的sd的基因集
  hc=hclust(dist(t(n))) 
  clus = cutree(hc, 4)
  group_list=as.factor(clus)
  table(group_list) ##这个聚类分组信息是针对top1000的sd的基因集的，和针对全部基因集的分组结果不一样
  table(group_list,df$g) ## 其中 df$g 是前面步骤针对全部表达矩阵的层次聚类结果。
  
  ## 下面针对本次挑选1000个基因的表达矩阵的层次聚类结果进行热图展示。
...
###先对整个基因集聚类分组，再对top1000的sd的基因集画图
###和选取top1000的sd的基因集，聚类分组再画图，结果聚类分组信息不同
###两次的聚类分组信息不同，画出的图不同
